...
# tweets request
# mail_tweets = api.user_timeline(screen_name='MailOnline', count=15)

# write results on file
with open('news-mailonline.txt', 'a', encoding="utf8") as outfile:
    for tweet in tweepy.Cursor(api.user_timeline, screen_name='MailOnline').items():
        ...
        if not 'RT' in tweet.text:
            ...
            with open("news-mailonline.txt", encoding="utf8") as tweets_file:
            ...
            if not tweet_present:
                ...
                try:
                    ...
                    for par in story_par:
                        if par.attrs == {'class': ['mol-para-with-font']}:
